...
def train_one_epoch(dataloader:Iterable,  model:torch.nn.Module,
                    ...
                    writer: SummaryWriter, epoch: int, mod, args, task_key=None):
    model.train()
    batches_start = datetime.now()
    for i_batch, (imgs, labs) in enumerate(dataloader):
        imgs = imgs.to(args.device)
        labs = labs.to(args.device)
        outs, dist = model(imgs, task_key=task_key,mod=mod)
        if True:
...
def test(dataloader, model, criterion, writer, epoch, mod, args, task_key=None):
